...
import {BasicRetailistJBParams, Tiered721PayAllocatorRetailistJBDeployer} from "./Tiered721PayAllocatorRetailistJBDeployer.sol";

/// @notice A contract that facilitates deploying a basic Retailist treasury that also can mint tiered 721s via a Croptop proxy.
contract CroptopTiered721PayAllocatorRetailistJBDeployer is Tiered721PayAllocatorRetailistJBDeployer {
    ...
    /// @param _publisher The Croptop publisher that facilitates the permissioned publishing of NFT posts to a Juicebox project.
    ...
    ) Tiered721PayAllocatorRetailistJBDeployer(_directory, _delegateDeployer, _controller, _terminal) {
        publisher = _publisher;
        croptopPermissionIndexes.push(JB721Operations.ADJUST_TIERS);
    }

    /// @notice Deploy a project with basic Retailism constraints that includes Tiered 721s that the Croptop publisher can facilitate permissioned posts to, and also calls other pay delegates that are specified.
